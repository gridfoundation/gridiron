...
import (
	...
	gridironapp "github.com/gridfoundation/gridiron/app"
	sharedtypes "github.com/gridfoundation/gridiron/shared/types"
...
func (suite *IrcTestSuite) SetupTest() {
	...
	suite.coordinator = ibctesting.NewCoordinatorWithConsensusType(suite.T(), []string{exported.Furyint,
		exported.Tendermint,
		exported.Furyint})
...
func (suite *IrcTestSuite) FinalizeState() (err error) {
	disputePeriodInBlocks := suite.hubChain.App.(*gridironapp.App).RollappKeeper.DisputePeriodInBlocks(suite.hubChain.GetContext())
...
// their own SimApp.
func GetHubSimApp(chain *ibctesting.TestChain) *gridironapp.App {
	app, ok := chain.App.(*gridironapp.App)
...
func SendMsgs(furyHubChain *ibctesting.TestChain, sequencerAccount ibctesting.SenderAccount, msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	furyHubChain.Coordinator.UpdateTimeForChain(furyHubChain)

	_, r, err := simapp.SignAndDeliver(
		furyHubChain.T,
		furyHubChain.TxConfig,
		furyHubChain.App.GetBaseApp(),
		furyHubChain.GetContext().BlockHeader(),
		msgs,
		furyHubChain.ChainID,
