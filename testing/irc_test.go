...
// TestConnection create connection between furyint rollapp and gridiron hub chain
func (suite *IrcTestSuite) SetupConnection(rollappChain *ibctesting.TestChain, hubChain *ibctesting.TestChain) *ibctesting.Path {
	// create chains, chainA is furyint and chainB is gridiron hub
	path := ibctesting.NewPath(rollappChain, hubChain)
	// gridiron hub chain
	gridironEndpoint := path.EndpointB
	// rollapp furyint chain
	...
	bds := &rollappChain.TestChainClient.(*FuryintTestChainClient).bds
	// update rollapp state
	err = suite.UpdateRollappState(rollappEndpoint.Chain.ChainID, bds)
	// create the client of gridiron on the rollapp chain
	err = rollappEndpoint.CreateClient()
	suite.Require().Nil(err)
	// update rollapp state
	err = suite.UpdateRollappState(rollappEndpoint.Chain.ChainID, bds)
	// create the client of the rollapp on gridiron
	err = gridironEndpoint.CreateClient()
	suite.Require().Nil(err)
	...
	err = gridironEndpoint.ConnOpenTry()
	suite.Require().Nil(err)
	...
	err = gridironEndpoint.ConnOpenConfirm()
	suite.Require().Nil(err)
	...
	gridironEndpoint.ChannelConfig.PortID = ibctransfertypes.PortID
	gridironEndpoint.ChannelConfig.Version = ibctransfertypes.Version
	...
	err = gridironEndpoint.ChanOpenTry()
	suite.Require().Nil(err)
	// send ack to to rollapp
	...
	err = suite.UpdateRollappState(rollappEndpoint.Chain.ChainID, bds)
	// send confirmation to the hub
	err = gridironEndpoint.ChanOpenConfirm()
	suite.Require().Nil(err)
...
func (suite *IrcTestSuite) TestHandleMsgMiddlewareTransfer() {
	...
	rollapp1bds := &suite.rollapp1.TestChainClient.(*FuryintTestChainClient).bds
	rollapp2bds := &suite.rollapp2.TestChainClient.(*FuryintTestChainClient).bds
...
// TestSuite initialize furyint rollapp and gridiron hub chain
