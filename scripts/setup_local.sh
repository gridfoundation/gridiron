...
TOKEN_AMOUNT=${TOKEN_AMOUNT:-"1000000000000ufury"} #1M FURY (1e6fury == 1e12ufury)
STAKING_AMOUNT=${STAKING_AMOUNT:-"670000000000ufury"} #67% is staked (inflation goal)

# Validate gridiron binary exists
export PATH=$PATH:$HOME/go/bin
if ! command -v furyd > /dev/null; then
  make install

  if ! command -v furyd; then
    echo "gridiron binary not found in $PATH"
    exit 1
  fi
fi

# Verify that a genesis file doesn't exists for the gridiron chain
...
# Create and init gridiron chain
...
read -r answer
if [ ! "$answer" != "${answer#[Nn]}" ] ;then
  furyd add-genesis-account "$(furyd keys show "$KEY_NAME" -a --keyring-backend test)" "$TOKEN_AMOUNT"
  furyd gentx "$KEY_NAME" "$STAKING_AMOUNT" --chain-id "$CHAIN_ID" --keyring-backend test
