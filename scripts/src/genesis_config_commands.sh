...
set_gov_params() {
    ...
    jq '.app_state.gov.deposit_params.min_deposit[0].denom = "ufury"' "$GENESIS_FILE" > "$tmp" && mv "$tmp" "$GENESIS_FILE"
...
set_misc_params() {
    jq '.app_state.crisis.constant_fee.denom = "ufury"' "$GENESIS_FILE" > "$tmp" && mv "$tmp" "$GENESIS_FILE"
}
